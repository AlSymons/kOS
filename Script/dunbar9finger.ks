// | Dunbar9finger's sky crane code
// | uses an out of date version of kOS, this is for study
...
//////////////////////////////////////////
 ...
 //KOS
  ...
  if sanityOK {
    ...
    set groundHeight to 0.  // Height of ground above sea level.
    set pgroundHeight to 0.  // previous Height of ground above sea.
    ...
    set tfE to 9999999. set tfN to 9999999. set tfU to 9999999. // east,north,up vector //haxnote: ???????
    lock absoluteSpeed to (tfE^2 + tfN^2 + tfU^2) ^ 0.5 .
    ...
    loopedOnce off.
    when bodyAtmSea = 0 or (loopedOnce and (absoluteSpeed < petermv) and usepe <= bodyMaxElev ) then {
    ...
    if bodyAtmSea > 0 {
      ...
      lock fdrag to 0.5*( (atmToPres*bodyAtmSea) * e^( (0-altitude)/bodyAtmScale) )*absoluteSpeed^2*vdrag*0.008*mass.
    ...
    until beDone {
      ...
      // ...................................................
      // set absoluteSpeed to (tfE^2 + tfN^2 + tfU^2) ^ 0.5 .
      set altRadar to alt:radar .
      set altSea to altitude .
      set spdSurf to surfacespeed. //haxnote: this is now VESSEL:GROUNDSPEED
      ...
      if altRadar = altSea and altSea > 10000 {
        set altRadar to altSea - (bodyMaxElev/2).
      }.
      print "              " at (22,1).
      print ( round( altRadar * 10 ) / 10 ) + " m" at (22,1).
  
  
      set groundHeight to (altSea-altRadar).
      set slope to 0.
      ...
      if spdSurf > 0.1 {
        set slope to (groundHeight-pgroundHeight)/(dTime*spdSurf).
      }
      set pgroundHeight to groundHeight.
      print "                        " at (22,10).
      ...
      print "            " at (22,8).
      print ( round( absoluteSpeed * 100 ) / 100 ) + " m/s" at (22,8).
  
      if altRadar < descendBot*3 {
        ...
        if dMode = 1 or dMode = 3 { set absvsup to absoluteSpeed + 1.0 . }.
      }.
      ...
      if dMode = 0 and altRadar > 0 and altRadar < descendBot {
      ...
      if dMode = 1 {
        set pDesSpd to 5 * ( altRadar - descendBot ) / descendBot  .
        if needNewAG9 = 1 {
      ...
      if dMode = 0 and altRadar > descendBot {
        ...
        set guessMoreH to (bodyMaxElev-groundHeight).
        if guessMoreH > altRadar { set guessMoreH to 0. }.
        // Height to use to calculate how much distance I have available to stop:
        set H to (altRadar-descendBot) - (guessMoreH*sinsteerup). 
        ...
        if periapsis > 0 and petermv > (absoluteSpeed*1.3) {
          set pDesSpd to 0.
        }.
        ...
        if  bodyAtmSea > 0 and absoluteSpeed > petermv*1.25 and stopdist < (altRadar-descendBot)/cossteerup {
      ...
      set spd to absoluteSpeed.
      if verticalspeed > 0.0 { set spd to 0 - absoluteSpeed. }.
      ...
      if altRadar < (descendBot*3) and spd > (pDesSpd*2) {
